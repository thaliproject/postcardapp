var EventEmitter = require('events').EventEmitter;
var inherits = require('util').inherits;

/**
 * Creates a new instance of the ThaliEmitter which is an EventEmitter to call the underlying native layer.
 */
function ThaliEmitter() {
  EventEmitter.call(this);
  this._isBroadcasting = false;
  this._connections = {};
  this._init();
}

inherits(ThaliEmitter, EventEmitter);

ThaliEmitter.events = {
  PEER_AVAILABILITY_CHANGED: 'peerAvailabilityChanged',
  NETWORK_CHANGED: 'networkChanged'
};

ThaliEmitter.prototype._init = function () {
  var self = this;

  function registerMobilePeerEvent(eventName) {
    function emitEvent(eventName) {
      return function handler(arg) {
        self.emit(eventName, arg);
      };
    }

    Mobile(eventName).registerToNative(emitEvent(eventName));
  }

  Object.keys(ThaliEmitter.events)
    .forEach(function (key) {
      registerMobilePeerEvent(ThaliEmitter.events[key]);
    });
};

/**
* Starts broadcasting with the given device name, port and a callback
* @param {String} deviceName the device name to broadcast.
* @param {Number} port the port number to broadcast.
* @param {Function} cb the callback which returns an error if one has occurred.
*/
ThaliEmitter.prototype.startBroadcasting = function(deviceName, port, cb) {
  
  console.log('ThaliEmitter.prototype.startBroadcasting');
  
  if (this._isBroadcasting) { throw new Error('Thali is already broadcasting'); }
  Mobile('StartBroadcasting').callNative(deviceName, port, function (err) {
    
    console.log('callNative(deviceName, port, function (err)');
    
    if (err) {
      console.log('callNative - if (err)');
      cb(new Error(err));
    } else {
      console.log('callNative - else');
      this._isBroadcasting = true;
      cb();
    }
  }.bind(this));
};


//=============================
//works
/*ThaliEmitter.prototype.getPublicKeyHash = function(cb) {
  console.log('ThaliEmitter.prototype.getPublicKeyHash');
  
  var cryptomanager = require('./thalicryptomanager');
  cryptomanager.getPublicKeyHash(function (publicKeyHash) {
    console.log('getPublicKeyHash(function (publicKeyHash)');
    cb(publicKeyHash);
  }.bind(this));
};*/

//doesn't work
/*ThaliEmitter.prototype.addCurrentAddress = function(db, currentAddrEntry, cb) {
  console.log('ThaliEmitter.prototype.addCurrentAddress');

  db.allDocs(({
    include_docs: true, 
    attachments: true
  }).then(function (result) {
    console.log('allDocs result: ', result);
    var currentAdddrFound = false;
    result.rows.forEach(function(element) {
      console.log('allDocs each element: ', element);
      if(element.doc.content == currentAddrEntry) {
        console.log('found current device address: ', element.doc.content);
        currentAdddrFound = true;
        cb(null);
      }
    });
    //if(!currentAdddrFound) {
      console.log('did not find current device address, adding it now');
      db.put({_id: currentAddrEntry, author: currentAddrEntry, content: currentAddrEntry})
        .then(function (response) {
          console.log('db.put response: ', response);
          cb(null);
        })
        .catch(function (err) {
          console.log('db.put err: ', err);
          cb(err);
        });
    //}
  }).catch(function (err) {
    console.log('allDocs err: ', err);
    cb(err);
  }).bind(this));
};*/
//=============================



/**
* Starts broadcasting the availability of the current device.
* @param {Function} cb the callback which returns an error if one has occurred.
*/
ThaliEmitter.prototype.stopBroadcasting = function(cb) {
  
  console.log('ThaliEmitter.prototype.stopBroadcasting');
  
  if (!this._isBroadcasting) { throw new Error('Thali has already stopped broadcasting'); }
  Mobile('StopBroadcasting').callNative(function (err) {
    console.log('callNative(function (err)');
    if (err) {
      console.log('callNative - if (err)');
      cb(new Error(err));
    } else {
      console.log('callNative - else');
      this._isBroadcasting = false;
      cb();
    }
  }.bind(this));
};

/**
* Connects to the given peer by the given peer identifier.
* @param {String} peerIdentifier the peer identifier of the device to connect to.
* @param {Function} cb the callback which returns an error if one occurred and a port number used for synchronization.
*/
ThaliEmitter.prototype.connect = function (peerIdentifier, cb) {
  if (this._connections[peerIdentifier] !== undefined) { throw new Error('This peer identifier has already been connected'); }
  Mobile('Connect').callNative(peerIdentifier, function (err, port) {
    if (err) {
      cb(new Error(err));
    } else {
      this._connections[peerIdentifier] = port;
      cb(null, port);
    }
  }.bind(this));
};

/**
* Disconnects from the given peer by the peer identifier. Note if the peer has already been disconnected, no error should be thrown.
* @param {String} peerIdentifier the peer identifier of the device to disconnect from.
* @param {Function} cb the callback which returns an error if one occurred.
*/
ThaliEmitter.prototype.disconnect = function (peerIdentifier, cb) {
  if (this._connections[peerIdentifier] === undefined) { throw new Error('This peer identifier is not connected'); }
  Mobile('Disconnect').callNative(peerIdentifier, function (err) {
    if (err) {
      cb(new Error(err));
    } else {
      delete this._connections[peerIdentifier];
      cb();
    }
  }.bind(this));
};

module.exports = ThaliEmitter;